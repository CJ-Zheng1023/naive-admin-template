<template>
  <n-popover trigger="click" placement="bottom" display-directive="show" content-style="padding: 0;">
    <template #trigger>
      <n-button>
    ...
    <template #header>
      <div class="search">
        <n-input v-model:value="input" type="text" placeholder="请输入ICON名称" :maxlength="60" clearable />
      </div>
    </template>
    <template #default>
...
import { iconKeys } from '@/plugins/carbon'
...
const { value = null } = defineProps<{
  value?: IconKeyType | null
}>()
const emit = defineEmits(['update:value'])
const input = ref('')
const inputDebounced = refDebounced(input, 500)
...
const filteredIconNames = computed(() => {
  return iconKeys.filter(item => regExpTest(inputDebounced.value, item))
})
// 根据分页筛选图标
